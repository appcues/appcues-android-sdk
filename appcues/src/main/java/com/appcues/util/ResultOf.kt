...
internal inline fun <reified Success, reified Failure> ResultOf<Success, Failure>.doIfSuccess(callback: (value: Success) -> Unit) = apply {
    if (this is ResultOf.Success) {
        callback(value)
    }
}

internal inline fun <reified Success, reified Failure> ResultOf<Success, Failure>.doIfFailure(callback: (reason: Failure) -> Unit) = apply{
